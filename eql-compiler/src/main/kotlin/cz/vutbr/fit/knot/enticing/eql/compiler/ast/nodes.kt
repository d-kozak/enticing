...
data class DummyRoot(val children: MutableList<DummyLeaf> = mutableListOf()) : AstNode {
    override fun deepCopy(): AstNode = DummyRoot(children.map { it.deepCopy() as DummyLeaf }.toMutableList())
}

data class DummyLeaf(val queryInterval: Interval, val documentIntervals: MutableList<Interval>) : AstNode {
    override fun deepCopy(): AstNode = DummyLeaf(queryInterval, documentIntervals.toMutableList())
}
