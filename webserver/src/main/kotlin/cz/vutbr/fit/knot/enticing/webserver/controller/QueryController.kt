...
class QueryController(private val queryService: QueryService) {
    ...
    @GetMapping
    fun query(@RequestParam query: String, @RequestParam settings: Long): List<Match> = queryService.query(URLDecoder.decode(query, "UTF-8"), settings)

    @PostMapping("/context")
    fun context(@RequestBody query: ContextExtensionQuery): Match = queryService.context(query)
