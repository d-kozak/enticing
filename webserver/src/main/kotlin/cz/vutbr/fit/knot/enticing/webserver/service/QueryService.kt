...
class QueryService {
    ...
    fun context(query: ContextExtensionQuery): ExtendedSnippet = Snippet(query.collection, 42, query.location.toInt(), query.size.toInt() + 10, "http://www.google.com", "title", Payload.FullResponse.Annotated(AnnotatedText(loremOneSentence, emptyMap(), emptyList(), emptyList())), Math.random() > 0.4) with ExtraInfo(query.host)

    fun document(query: ExtendedDocumentQuery): ExtendedDocument = dummyDocument
}